...
async def _wait_for_postgres(engine: AsyncEngine, timeout: float = 3.0) -> None:
    ...
    """
    print(f"[tests.db] Waiting for Postgres at {settings.database_url} (timeout={timeout}s)")
    try:
        ...
        raise RuntimeError("Postgres not reachable; check DATABASE_URL and container health") from exc
